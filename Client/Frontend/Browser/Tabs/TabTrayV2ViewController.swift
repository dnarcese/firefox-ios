...
extension TabTrayV2ViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        dateFormatter.locale = Locale(identifier: Locale.current.languageCode ?? "en_US")
        
        let dateIntervalFormatter = DateIntervalFormatter()
        dateIntervalFormatter.dateStyle = .medium
        dateIntervalFormatter.timeStyle = .none
        dateIntervalFormatter.locale = Locale(identifier: Locale.current.languageCode ?? "en_US")
